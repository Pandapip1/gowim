...
func (l *layout) writePVD(w *sectorWriter) error {
	...
	if err := writeDirectoryRecordV(s[156:190], isoView, l.dirs[0], selfRecord, l.b.opts); err != nil {
		return err
	}
...
// writePathTable emits one ECMA-119 Path Table. See writePathTableV.
func (l *layout) writePathTable(w *sectorWriter, msb bool) error {
	return l.writePathTableV(w, isoView, msb)
}

// writePathTableV emits one Path Table (ECMA-119 9.4) for the given
// hierarchy, in Type M byte order if msb is set and Type L order otherwise
// (6.9.2).
//
...
// satisfy 6.9.1; the Joliet Path Table Group reuses that same order rather
// than independently sorting by UCS-2 identifier (see joliet.go's file
// comment for why that is a deliberate, documented simplification).
func (l *layout) writePathTableV(w *sectorWriter, v hierarchyView, msb bool) error {
	buf := make([]byte, 0, v.pathTableSize(l))
	for _, d := range l.dirs {
		id := v.pathTableID(d)
		rec := make([]byte, pathTableRecordLenBytes(len(id)))
		put711(rec[0:1], uint8(len(id))) // 9.4.1 Length of Directory Identifier
		put711(rec[1:2], 0)              // 9.4.2 Extended Attribute Record length
		if msb {
			put732(rec[2:6], v.extent(d))    // 9.4.3 Location of Extent
			put722(rec[6:8], parentIndex(d)) // 9.4.4 Parent Directory Number
		} else {
			put731(rec[2:6], v.extent(d))
...
// writeDirectoryRecord formats one ECMA-119 Directory Record. See
// writeDirectoryRecordV.
func writeDirectoryRecord(dst []byte, n *node, kind recordKind, opts Options) error {
	return writeDirectoryRecordV(dst, isoView, n, kind, opts)
}

// writeDirectoryRecordV formats one Directory Record (ECMA-119 9.1) for the
// given hierarchy into dst, which must be exactly LEN_DR bytes long.
//
// The "." and ".." records use the single reserved (00)/(01) byte
// identifiers in both hierarchies: joliet.c's generate_one_joliet_directory
// special-cases exactly these two names to write a single byte rather than
// a UCS-2-converted name (`directory_buffer[dir_index++] = 0;` for ".",
// `= 1` for ".."), so hierarchyView is not consulted for the identifier in
// those two cases, only for the extent/length of the directory it names.
func writeDirectoryRecordV(dst []byte, v hierarchyView, n *node, kind recordKind, opts Options) error {
	var (
		id     []byte
		extent uint32
		length uint32
	...
	switch kind {
	case selfRecord:
		id, extent, length, flags, mtime = selfID, v.extent(n), v.length(n), dirFlag, n.modTime
	case parentRecord:
		p := n.parent
		if p == nil {
		...
		id, extent, length, flags, mtime = parentID, v.extent(p), v.length(p), dirFlag, p.modTime
	default:
		id, mtime = v.fileID(n), n.modTime
		if n.isDir {
			extent, length, flags = v.extent(n), v.length(n), dirFlag
		} else {
...
// selfID and parentID are the reserved single-byte File Identifiers of
// ECMA-119 6.8.2.2 for the "." and ".." Directory Records, shared by both
// the ECMA-119 and the Joliet hierarchy (see writeDirectoryRecordV).
var (
	selfID   = []byte{0}
	parentID = []byte{1}
)
...
func putDirectoryRecord(dst []byte, id []byte, extent, length uint32, flags byte, mtime time.Time, opts Options) error {
	fi := len(id)
	lenDR := 33 + fi + (1 - fi%2)
	if len(dst) != lenDR {
...
// writeDirectories emits every ECMA-119 directory's extent. See
// writeDirectoriesV.
func (l *layout) writeDirectories(w *sectorWriter) error {
	return l.writeDirectoriesV(isoView, w)
}

// writeDirectoriesV emits every directory's extent in the given hierarchy,
// in the order the sizing pass allocated them (l.dirs, shared by both
// hierarchies; see the node.jolietDirExtent doc comment).
func (l *layout) writeDirectoriesV(v hierarchyView, w *sectorWriter) error {
	for _, d := range l.dirs {
		if err := l.writeDirectoryV(v, w, d); err != nil {
			return err
		}
	}
	return nil
...
func (l *layout) writeDirectoryV(v hierarchyView, w *sectorWriter, d *node) error {
	buf := make([]byte, 0, v.length(d))

	// 6.8.2.2: the first two records are "." and "..", in that order.
	self := make([]byte, 34)
	if err := writeDirectoryRecordV(self, v, d, selfRecord, l.b.opts); err != nil {
		return err
	}
	parent := make([]byte, 34)
	if err := writeDirectoryRecordV(parent, v, d, parentRecord, l.b.opts); err != nil {
		return err
	}
	buf = append(buf, self...)
	...
	for _, c := range d.children {
		if c.isDir {
			rec := make([]byte, directoryRecordLenV(v, c))
			if err := writeDirectoryRecordV(rec, v, c, childRecord, l.b.opts); err != nil {
				return err
			}
		...
		if c.isoHidden {
			...
			// still occupies extents in this image; nothing in either the
			// ECMA-119 or the Joliet hierarchy points at them, since both
			// share the same 32-bit Data Length field that made the file
			// too large to record in the first place.
			continue
		}
		...
		//
		// genisoimage itself never builds a multi-extent file (see
		// name.go's Level3 doc comment) so its Joliet tree gives no
		// evidence either way for this case; this package treats the two
		// hierarchies symmetrically here as the simpler, self-consistent
		// choice; like ECMA-119 Level 3 multi-extent itself, it is
		// UNVERIFIED against a real reader.
		for i, sec := range c.sections {
			...
			rec := make([]byte, directoryRecordLenV(v, c))
			if err := putDirectoryRecord(rec, v.fileID(c), sec.extent, sec.length, flags, c.modTime, l.b.opts); err != nil {
				return err
			}
			buf = appendRecord(buf, rec)
		}
	...
	if uint32(len(buf)) > v.length(d) {
		return fmt.Errorf("iso: internal error: %s directory %q needs %d bytes but was laid out as %d",
			v.name, d.hostName, len(buf), v.length(d))
	}
	if err := w.write(buf); err != nil {
		return err
	}
	...
	return w.zeroBytes(int(v.length(d)) - len(buf))
}
